...
export type InvoiceDirectoryPreset = { customerName?: string; product?: LocalProduct };
const money = (value: string) => `${new Intl.NumberFormat("fr-FR").format(Number(value))} GNF`;

export function LocalDirectoryView({ kind, onCreateInvoice }: { kind: DirectoryKind; onCreateInvoice?: (preset: InvoiceDirectoryPreset) => void }) {
  ...
  const saveItem = async (createInvoice = false) => {
    ...
    if (kind === "customers") {
      ...
      if (createInvoice) onCreateInvoice?.({ customerName: customer.name });
    } else {
      const unitPrice = contact.replace(/\s/g, "");
      if (!/^\d+$/.test(unitPrice)) { setMessage("Le prix doit être un montant entier en GNF."); return; }
      const product: LocalProduct = { id: crypto.randomUUID(), name: name.trim(), description: description.trim(), unitPriceMinor: unitPrice, vatRateBp: String(Number(vatRate || 0) * 100), unit: "unité", createdAt: Date.now() };
      await offlineDb.products.add(product);
      if (createInvoice) onCreateInvoice?.({ product });
    }
  ...
  return <section className={`directory-layout ${mobileFormOpen ? "mobile-form-open" : ""}`}>
    <nav className="directory-mobile-flow" aria-label="Préparer une facture"><button className={kind === "customers" ? "active" : ""} type="button" onClick={() => window.dispatchEvent(new CustomEvent("facturepro:navigate", { detail: "Clients" }))}>1. Clients</button><button className={kind === "products" ? "active" : ""} type="button" onClick={() => window.dispatchEvent(new CustomEvent("facturepro:navigate", { detail: "Produits" }))}>2. Produits</button><button type="button" onClick={() => onCreateInvoice?.({})}>3. Facture</button></nav>
